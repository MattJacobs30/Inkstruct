...
def recursively_merge_short_lines(strokes_list, merge_short_line_dist, min_long_line_for_merge_ratio, short_merge_threshold, stray_mark_threshold):
    ...
    # Create a mutable list of stroke info dictionaries.
    # Each dict will hold original index, current points, current length, and a 'merged' flag.
    # The 'merged' flag indicates if this stroke has been consumed by a merge (i.e., will be removed).
    current_stroke_info = []
    for i, s in enumerate(strokes_list):
        current_stroke_info.append({
            ...
            'merged': False # True if this stroke has been absorbed by another
        })

    # Separate strokes into potential sources (short) and potential targets (long)
    short_strokes_to_merge = []
    long_strokes_for_merge_endpoints_data = [] # For building KDTree of *target* endpoints

    for info_dict in current_stroke_info:
        if len(info_dict['points']) < 1:
            continue # Skip empty strokes

        # If a stroke's current length is below the threshold, it's a candidate to be merged (source)
        if info_dict['length'] < short_merge_threshold:
            short_strokes_to_merge.append(info_dict)
        
        # Only "long" strokes (length >= short_merge_threshold) are candidates to *receive* merges (targets)
        # Add their endpoints to the list for the KDTree.
        if info_dict['length'] >= short_merge_threshold:
    ...
    if not short_strokes_to_merge:
        # No short strokes to merge, so no merges will occur in this iteration.
        return strokes_list, False

    # Build KDTree ONLY from endpoints of "long" strokes.
    ...
    if len(long_stroke_endpoint_coords) > 0:
        ...
        # No long strokes to merge into, so no merges can occur.
        return strokes_list, False

    merges_occurred_in_this_iteration = False

    # Iterate through the short strokes, trying to merge each one
    for source_stroke_info in short_strokes_to_merge:
        # If this short stroke has already been consumed by a merge in this iteration, skip it.
        if source_stroke_info['merged']:
            continue
            
        short_original_idx = source_stroke_info['original_idx']
        short_stroke_points_to_add = list(source_stroke_info['points']) # Get a mutable copy of points
        ...
        # Check both ends of the short stroke for potential merges with long strokes
        for short_test_pt_idx, short_test_pt_xy in enumerate([short_start_xy, short_end_xy]):
            ...
            for dist, idx_in_data in zip(dists, idxs):
                ...
                # Extract info about the potential target endpoint from the long_strokes_for_merge_endpoints_data
                ...
                # A stroke cannot merge into itself
                if target_original_idx == short_original_idx:
                    continue
                
                # Find the actual stroke info dict for the target stroke (from the main current_stroke_info list)
                ...
                # IMPORTANT: Ensure the target stroke has not been marked as merged/consumed by another short stroke
                if target_stroke_dict is None or target_stroke_dict['merged']:
                    continue 

                target_len = target_stroke_dict['length']
                
                # Check the length ratio condition: target must be significantly longer than the short stroke
                if target_len < short_len * min_long_line_for_merge_ratio:
                    continue

                # If this candidate is better than previous ones, update best_merge_candidate
                if dist < min_overall_dist:
                    min_overall_dist = dist
                    
                    # Determine if the short stroke needs to be reversed
                    # If short_test_pt_idx == 0 (short_start_xy) and connects to target's start (is_target_start_point),
                    # OR if short_test_pt_idx == 1 (short_end_xy) and connects to target's end (NOT is_target_start_point),
                    # then reversing the short stroke is necessary to align its 'other' end with the target.
        ...
        # If a valid merge candidate (a 'long' stroke) was found for the current short stroke
        if best_merge_candidate:
            ...
            # Mark the source (short stroke) as merged/consumed so it's removed from the final list
            source_stroke_info['merged'] = True # Set the flag in the current_stroke_info list

            # Reverse the short stroke points if needed for correct concatenation
            if is_short_reversed:
                short_stroke_points_to_add.reverse()

            # Find the target stroke's info dict in the main current_stroke_info list and modify its points
            for target_s_info in current_stroke_info:
                if target_s_info['original_idx'] == target_original_idx:
                    if which_end_of_target == "start":
                        # Prepend: If last point of added segment matches first of target, avoid duplicate
                    ...
                    else: # which_end_of_target == "end"
                        # Append: If first point of added segment matches last of target, avoid duplicate
                    ...
                    # IMPORTANT: Update the length of the modified target stroke
                    target_s_info['length'] = compute_polyline_length(target_s_info['points'])
                    break
    
    # Reconstruct the new list of strokes from the modified current_stroke_info
    new_strokes_list = []
    for s_info in current_stroke_info:
        # Only include strokes that were NOT merged (consumed)
        if s_info['merged']: 
            continue
        
        # Also, filter out any remaining strokes that are now considered 'stray marks'
        # (i.e., too short and couldn't be merged into a longer line)
        if s_info['length'] >= stray_mark_threshold:
            if len(s_info['points']) > 0: # Ensure it's not an empty list of points
